...
import Server.labWorkClass.Difficulty
...
object ClientCmds {
    fun prepare(msg: String): String{
        ...
        if (tokens.size == 2)
        {
            ...
            if (listOf("insert", "replace_if_greater", "replace_if_lower").contains(cmd)) {
                IOManager.send("Введите команду верно - $cmd key, где key - ключ элемента коллекции")
            }
            else if (cmd == "update"){
    ...
    private fun collectText(file: String):String{
        try {
            IOManager.opendFiles = mutableListOf(file)
            val path = Paths.get("files", IOManager.opendFiles[0])
