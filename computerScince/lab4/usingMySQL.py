...
def drop_database(connection):
    cursor = connection.cursor()
    try:
        cursor.execute(f"DROP DATABASE games_db_mysql")
...
connection = create_connection("localhost", "root", "passwd")

create_database_query = "CREATE DATABASE games_db_mysql"
...
connection = create_connection("localhost", "root", "passwd!", "games_db_mysql")
...
FROM
  ...
  GROUP BY played_hours, users.nickname
"""
