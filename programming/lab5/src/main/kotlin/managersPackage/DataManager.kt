...
object DataManager {
    ...
    fun load(): HashMap<String, LabWork> {
        ...
        try {
            ...
            if (path.exists()) {
                ...
                println("Файл не найден: $filedir/$filename")
                println("Программа будет запущена без исходных данных")
            }
        } catch (e: Exception) {
            println("Ошибка при чтении файла: $filedir/$filename ${e.message}")
        }
        return newList
    }
    /**
    ...
    fun save(labWorks: HashMap<String, LabWork>) {
        ...
        try {
            ...
            println("Данные успешно записаны в файл data.csv")
        } catch (e: Exception) {
    ...
    private fun parsefile(fields: List<String>): LabWork {
        ...
        return LabWork(id, name, coordinates, creationDate, minimalPoint, personalQualitiesMinimum, difficulty, author)
    }
}